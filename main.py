...
for recording in recordings:
    ...
    subprocess.run(['ffmpeg', '-i', recording["path"], '-vf', 'fps=60', './images/frame%06d.png'])

    # 2
    # split each frame into 2 parts and safe with id_0 id_1
    for i, filename in enumerate(os.listdir('images')):
        if filename.endswith('.png'):
            subprocess.run(['convert', '-crop', '50%x100%', f'images/{filename}', f'./split_images/split_{i}_%d.png'])

    # 3
    # split each frame into 2 parts and safe with id_0 id_1
    def process_image(filename):
        if filename.endswith('.png'):
            i = filename.split('.')[0]  # Assuming filename is '0.png', '1.png', etc.
            subprocess.run(['convert', '-crop', '50%x100%', f'images/{filename}', f'./split_images/split_{i}_%d.png'])
    ...
    for filename in os.listdir('split_images'):
        if filename.startswith('split_'):
            img_id, split_id = filename.split('.')[0].split('_')[1:]  # Extract the id and split number
            with Image.open(f'split_images/{filename}') as img:
                try:
                    results = decode(img)
                    qrcodes = [result.data.decode() for result in results if result.type == 'QRCODE']
    ...
    df['difference'] = pd.to_numeric(df['data_point_0']) - pd.to_numeric(df['data_point_1'])

    df['difference_from_mean'] = round(df['difference'] - df['difference'].mean())
    ...
    df.to_csv(f'./result/{recording["filename"]}', index=False)

    # move file from recordings to processed
    os.rename(recording['path'], f'./processed/{recording["filename"]}.csv')
    print(df)
